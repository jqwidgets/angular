...
function getOrderData(count) {
  const orderData: any = [],
    ...
    productPrices = [
      47.59, 7.48, 64.59, 29.99, 28.99, 25.49, 17.03, 10.14, 136.88, 7.73,
      71.33, 6.99,
    ],
    countryCodes: any = getCountriesCodesData();
  ...
  for (let i = 0; i < count; i++) {
    ...
    const order: any = {
      ...
      date: new Date(
        new Date().getTime() - Math.floor(Math.random() * 9 + 1) * 86400000
      ),
    ...
    order['profit'] = parseFloat(
      (((Math.floor(Math.random() * 30) + 9) / 100) * order.total).toFixed(2)
    );
...
@Component({
  ...
  standalone: true,
  imports: [jqxGridModule, GridModule],
  standalone: true,
  ...
  columns: any[] = [
    ...
    },
    {
      ...
      cellsrenderer: function (row, dataField, value, html, column, data) {
        return (
          '<div style="align-items: center; height: 100%; display: flex;"><img class="flag" style="width: 15px; height: 10px;" src="./flags/' +
          data.countryCode +
          '.svg"/>' +
          value +
          '</div>'
    ...
    },
    {
      ...
      dataField: 'status',
      width: 150,
      cellclassname: function (index: number, dataField: string, value: any) {
        if (value === 'Delivered') {
          return 'delivered';
